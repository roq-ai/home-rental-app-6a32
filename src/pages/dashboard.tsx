...
function DashboardPage() {
  ...
  const propertyStat = useSWR<PaginatedInterface<PropertyInterface>>(
    () => `/properties`,
    () => getProperties({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; isLoading: boolean }[] = [
    {
      title: 'Properties',
      result: propertyStat?.data?.totalCount,
      isLoading: propertyStat.isLoading,
    },
    {
      title: 'Bookings',
